...
float warp(vec2 p) {
    ...
    float mouseX = iMouse.x*0.005;
    
    float mouseY = iMouse.y*0.005;
    
  vec2 v = vec2(fbm1(p), fbm1(p+0.5*vec2(1.0, 1.0)));
  
  rot(v, 1.0+iTime*1.1+mouseX);
  
  vec2 vv = vec2(fbm2(p + 3.7*v), fbm2(p + -2.7*v.yx+0.7*vec2(1.0, 1.0)));

  rot(vv, -1.0+iTime*1.21315+mouseY);
...
vec3 postProcess(vec3 col, vec2 q)  {
  ...
  vec3 baseCol = vec3(0.6, 0.6, 0.8)+0.2;
  ...
  vec3 col = vec3(0.0);
  col += baseCol*pow(diff1, 9.0); //hilights
  col += 0.1*baseCol*pow(diff1, 0.1); // greys
